...
static int addone(int x) NOINLINE;
static int addone(int x) { return x + 1; }
static int (*fptr)(int) = addone;

NOINLINE void user_program() {
  canon_rc += 1;              /* start */
  if (fptr(41) != 42) canon_rc += 32;  /* indirect call, outside protected region */
  if (!setjmp_canon(&my_buf)) {
    canon_rc += 4;            /* call-inner path */
    if (fptr(100) != 101) canon_rc += 64;  /* indirect call inside protected region */
    inner();
  } else {
