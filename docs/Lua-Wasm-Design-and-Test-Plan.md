...
> **Status (2026-09-04): M3 in progress — architecture decided and proven; first ABI batch frozen.** The module-boundary question was settled empirically (`wasm/sharedmem_test.go`, gate green): the script module **imports the runtime module's exported memory** — one shared address space; data segments, stores, and pointers cross freely (verified in wazero), and `rt_*` calls are downward-only. Consequence, now ABI law: **the runtime never calls into script code** — whenever C must run Lua (function `__index`/`__newindex`, sort comparators, pcall into compiled functions) the runtime's own lvm interprets the proto, because script protos are real `Proto` structs in shared memory. Correctness never depends on the compiled path; compiled code is pure acceleration (compiled→compiled calls stay in the script module). Cross-module dispatch (perf tiering only) is an M8 concern. `runtime/rt_abi.h` freezes the contract (LUA_RT_ABI=1, status codes, Lua-5.1 tag constants incl. the collectable bit, OP_*-aligned arith codes, error protocol); `runtime/rt_abi.c` implements the first batch (version, set_state, mkvalue, intern, newtable, protected gettable/settable with oracle-format error staging) and is compiled into `lua51_sjlj.wasm` with 12 exports — the M2 conformance gate stays green, and the static asserts pin the 16-byte TValue layout. The emitter gained `ImportMemory` + data-into-imported-memory. **Remaining for M3 completion:** rest of the surface (arith, len, eq/lt/le, concat, closures/upvalues, call fallback, forprep), native unit tests with ASan/UBSan, the emitted-side seam smoke against the real runtime, the big.lua yield investigation (carried from M2), and the freeze review.
